...
(window["webpackJsonp"] = window["webpackJsonp"] || []).push([["products-products-module"], {
  ...
  "./src/app/products/products.page.ts": function srcAppProductsProductsPageTs(module, __webpack_exports__, __webpack_require__) {
    ...
    var src_app_globals__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(
    /*! src/app/globals */
    "./src/app/globals.ts");
    /* harmony import */
    ...
    var ProductsPage = /*#__PURE__*/function () {
      function ProductsPage(apiService, g, global, router, activatedRoute) {
      ...
      _createClass(ProductsPage, [{
        ...
        value: function ngOnInit() {
          ...
          this.activatedRoute.params.subscribe(function (params) {
            _this.id = params["id"];
            _this.global.id_category = parseInt(_this.id);

            _this.getCategoryName(_this.id);

            _this.productsAux = [];

            if (parseInt(_this.id, 10) === 0) {
              _this.productsAux = _this.g.fruits;
            } else if (parseInt(_this.id, 10) === 1) {
              _this.productsAux = _this.g.vegetables;
            } else if (parseInt(_this.id, 10) == 2) {
              _this.productsAux = _this.g.spices;
            } else if (parseInt(_this.id, 10) == 3) {
              _this.productsAux = _this.g.carnes;
            } else if (parseInt(_this.id, 10) == 4) {
              _this.productsAux = _this.g.cremeria;
            } else if (parseInt(_this.id, 10) == 5) {
              _this.productsAux = _this.g.otros;
            }

            if (_this.productsAux.length == 0) {
              _this.apiService.getProducts(_this.id).subscribe(function (response) {
                _this.submenus = response.submenus;
                _this.products = _this.submenus[0].products;

                _this.showProductsSubmenu(_this.submenus[0]);
              });
            } else {
              _this.apiService.getProducts(_this.id).subscribe(function (response) {
                _this.submenus = response.submenus;
              ...
              _this.products = _this.productsAux;

              _this.products.map(function (x) {
                var exist = _this.productsAux.filter(function (y) {
        ...
        key: "getStyle",
        value: function getStyle(product) {
        ...
        key: "showProductsSubmenu",
        value: function showProductsSubmenu(item) {
          var _this2 = this;

          var submenu_products = item.products;
          this.products.map(function (x) {
            var exist = _this2.productsAux.filter(function (y) {
              if (y.idproduct == x.idproduct) {
                return y;
              }
            });
          ...
          this.products.map(function (x) {
            var exist = submenu_products.filter(function (y) {
        ...
        value: function getCategoryName(id) {
          var result = this.categorias.filter(function (x) {
            if (x.id == id) {
              return x;
            }
          });
        ...
        value: function count(_count, idproduct, type) {
          for (var i = 0; i < this.products.length; i++) {
            if (this.products[i]["idproduct"] == idproduct) {
              if (type == "K") {
                if (_count == 1995) {
                  this.products[i]["kg"] = parseFloat(this.products[i]["kg"]) + parseFloat(this.products[i]["sum"]);
                } else if (_count == -1995) {
                  this.products[i]["kg"] = parseFloat(this.products[i]["kg"]) - parseFloat(this.products[i]["sum"]);
                } else {
                  this.products[i]["kg"] = parseFloat(this.products[i]["kg"]) + _count;
                }
                ...
                this.products[i]["pc"] = parseInt(this.products[i]["pc"], 10) + _count;
              ...
              this.products[i]["selectedWeight"] = this.products[i]["pc"] * this.products[i]["weight"] + parseFloat(this.products[i]["kg"]) * 1000;
              break;
            }
          }
    ...
    }), Object(tslib__WEBPACK_IMPORTED_MODULE_0__["__metadata"])("design:paramtypes", [src_app_services_api_service__WEBPACK_IMPORTED_MODULE_2__["ApiService"], src_app_globals__WEBPACK_IMPORTED_MODULE_3__["Globals"], _services_global_service__WEBPACK_IMPORTED_MODULE_5__["GlobalService"], _angular_router__WEBPACK_IMPORTED_MODULE_4__["Router"], _angular_router__WEBPACK_IMPORTED_MODULE_4__["ActivatedRoute"]])], ProductsPage);
    /***/
  }
}]);
